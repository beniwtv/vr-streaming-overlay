...
var WidgetsBoxNode = "MarginContainer/VBoxContainer/WidgetBox"
...
func _ready():
	...
	# Load default margin + Twitch chat widgets
	widgets.set_tree_from_array(DefaultSettings.get_default_setting("widgets/configuration"))
	_redraw_tree()

func set_active_overlay(overlay : Dictionary) -> void:
	current_overlay = overlay
	
	if overlay.has("widgets"): 
		widgets.set_tree_from_array(overlay["widgets"])
	else:
		widgets.set_tree_from_array(DefaultSettings.get_default_setting("widgets/configuration"))

	if current_widget_panel:
		# Unload the current panel if any
		current_widget_panel.queue_free()
		current_widget_panel = null

	_redraw_tree()

# Saves the widgets to the user's config file
func save_tree():
	current_overlay["widgets"] = widgets.get_tree_as_array()
...
func show_widget_options(widget_id):
	...
	current_widget_panel.set_receiver_node(self)
...
func add_widget_to_tree(parent, droppednear, droppedsection, widgetpath):
	...
	SignalManager.emit_signal("redraw_overlay", current_overlay)
...
func _redraw_tree():
	...
	for i in range(0, arrayWidgets.size()):
		...
		if metadata["data"]["parent"]:
			# Add remove button
			treeitem.add_button(0, load("res://ui/theme/remove.png"), -1, false, "Delete this widget")
