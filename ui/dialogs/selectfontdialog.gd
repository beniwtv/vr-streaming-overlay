...
func edit_font(font : Dictionary):
	...
	for item in get_node(font_list_node).get_item_count():
		if get_node(font_list_node).get_item_text(item) == font.name:
	...
	for item in get_node(font_size_node).get_item_count():
		if get_node(font_size_node).get_item_text(item) == str(font.size):
...
func _on_FontList_item_selected(index):
	font_options.file = get_node(font_list_node).get_item_metadata(index) + "/" + get_node(font_list_node).get_item_text(index)
	font_options.name = get_node(font_list_node).get_item_text(index)
	render_font()

func _on_FontSizeList_item_selected(index):
	font_options.size = int(get_node(font_size_node).get_item_text(index))
	render_font()
